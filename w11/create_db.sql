...
CREATE TABLE Driver (
    ...
    CONSTRAINT chk_driver_status CHECK (availability_status IN ('Available', 'Busy', 'Offline'))
);
...
CREATE TABLE Food_Order (
    ...
    driver_id           NUMBER,
    CONSTRAINT chk_order_status CHECK (status IN ('Pending','Confirmed','Preparing','Out for Delivery','Delivered','Cancelled')),
...
CREATE TABLE MenuItem (
    ...
    description         CLOB,
    availability_status VARCHAR2(20)    NOT NULL,
    CONSTRAINT chk_menuitem_status CHECK (availability_status IN ('Available', 'Unavailable')),
...
CREATE TABLE OrderItem (
    ...
    CONSTRAINT pk_orderitem PRIMARY KEY (order_id, order_item_id),
    CONSTRAINT chk_orderitem_qty CHECK (quantity > 0),
    CONSTRAINT fk_orderitem_order FOREIGN KEY (order_id) REFERENCES Food_Order(order_id),
    CONSTRAINT fk_orderitem_menuitem FOREIGN KEY (menu_item_id) REFERENCES MenuItem(menu_item_id)
);
...
CREATE TABLE Payment (
    ...
    CONSTRAINT fk_payment_order FOREIGN KEY (order_id) REFERENCES Food_Order(order_id)
);
